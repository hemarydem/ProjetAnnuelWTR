...
if(isset($_POST['levelName']) &&$_POST['option'] == 0) {
    $q = "SELECT name, threshold FROM LEVEL WHERE name LIKE ?";
    $req = $bdd->prepare($q);
    ...
    $q = "SELECT name, threshold FROM LEVEL WHERE threshold < ?";
    $req = $bdd->prepare($q);
  ...
  if(isset($_POST['levelName']) && isset($_POST['threshold']) &&$_POST['option'] == 3) {
    $q = "SELECT name, threshold FROM LEVEL WHERE name LIKE ? AND threshold < ?";
    $req = $bdd->prepare($q);
    $unKnowLevel = $_POST['levelName'].'%';
